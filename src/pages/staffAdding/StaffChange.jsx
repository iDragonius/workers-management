...
const StaffChange = () => {
    ...
    useEffect(() => {
        getEmployee(location.pathname.split('/').at(-1))
            .then(res=>{
    ...
    return (
        ...
        >
            ...
            <div>
                {data.status === 1  ? (
                    <>
                        <button  className={' py-2 px-4 text-white bg-green-600 rounded-md  font-semibold mr-5'}  onClick={()=> change(2)} >Accept</button>
                        <button  className={'py-2 px-4 text-white bg-red-600 rounded-md font-semibold'} onClick={()=> change(3)} >Reject</button>
                    </>
                )
                :
                    (
                        <Button label={'Update'} primary onClick={()=>change(2)} />
