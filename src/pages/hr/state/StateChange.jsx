...
import { addState, getAllStates, getState, updateState } from '../../../http/api/states.js'
...
const StateChange = () => {
    ...
    useEffect(()=>{
        getAllStates()
            .then(allStates=>{
                ...
                getState(location.pathname.split('/').at(-1))
                    .then((res=>{
                        setData({
                            name:res.data.name,
    ...
    const update = async () => {
        await updateState({...data, id: +location.pathname.split('/').at(-1)} )
    }
    const changeData = (e) => {
